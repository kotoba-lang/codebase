...
;; ---------------------------------------------------------------------------
;; filesystem — one provider, and the only one that cannot leave the JVM.
;;
;; Everything above this line is portable: the protocol, `store-schema` and
;; `store?` are what a ClojureScript caller needs in order to BE a backend.
;; What follows is `java.nio` and is spliced in only on the JVM, so a Worker
;; can implement `ICodebaseStore` without this file dragging a filesystem
;; into its build.
...
(defn coerce
  "A backend value passes through. On the JVM anything else is a path to the
  filesystem layout, which is what lets every existing `(store/get-block root
  cid)` caller keep working unchanged.

  On ClojureScript there is no filesystem to fall back TO, so a non-backend
  is refused by name instead of failing later inside a nil provider. A cljs
  caller supplies its own `ICodebaseStore`; that is the whole point of the
  seam."
  [root]
  (if (store? root)
    root
    #?(:clj (filesystem root)
       :cljs (throw (ex-info "codebase: no filesystem backend on this platform — pass an ICodebaseStore value"
                             {:problem :codebase/no-default-backend
                              :given (type root)})))))
